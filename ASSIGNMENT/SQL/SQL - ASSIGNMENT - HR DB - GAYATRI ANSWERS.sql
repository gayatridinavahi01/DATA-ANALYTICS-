...
SELECT CONCAT( FIRST_NAME,'  ', LAST_NAME) AS ' ', COMMISSION_PCT AS '  '    FROM EMPLOYEES 
...
WHERE   L.CITY IN  (SELECT CITY FROM LOCATIONS  WHERE FIRST_NAME = 'TRENNA');


select E.FIRST_NAME, D.DEPARTMENT_NAME , L.CITY, J.JOB_ID  
	from employees e  inner join departments d 
	 on e.department_id = d.department_id
...
AND l.city in (select l1.city from locations l1  join employees e1 where e1.first_name like 'TRENNA')  ;
